//
//  CollectionViewAlignedFlowLayout.swift
//  FSUIKitSwift
//
//  Created by VincentLee on 2025/1/14.
//
//  Inspired from: https://github.com/mischa-hildebrand/AlignedCollectionViewFlowLayout
//

import UIKit

/// An abstract protocol that defines an alignment.
fileprivate protocol Alignment {}
...
open class CollectionViewAlignedFlowLayout: FSCollectionViewAnimatedFlowLayout {
    ...
    /// A horizontal alignment used internally by `CollectionViewAlignedFlowLayout`
    /// to layout the items, after resolving layout direction specifics.
    fileprivate enum EffectiveHorizontalAlignment: Alignment {
        case left
        case right
        case justified
    }
    ...
    fileprivate struct AlignmentAxis<A: Alignment> {
    ...
    override open func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if horizontalAlignment != .justified {
            layoutAttributes.alignHorizontally(collectionViewLayout: self, forSectionAt: indexPath.section)
        }
    ...
    fileprivate func sectionInsetForSection(at section: Int) -> UIEdgeInsets {
    ...
    fileprivate func minimumInteritemSpacingForSection(at section: Int) -> CGFloat {
        if let collectionView = collectionView,
           let delegate = collectionView.delegate as? UICollectionViewDelegateFlowLayout,
    ...
    private func contentWidthForSection(at section: Int) -> CGFloat {
        guard let collectionView = collectionView else {
            return 0.0
        }
        let inset = sectionInsetForSection(at: section)
    ...
    fileprivate func alignmentAxisForSection(at section: Int) -> AlignmentAxis<HorizontalAlignment>? {
        let inset = sectionInsetForSection(at: section)
        switch effectiveHorizontalAlignment {
        case .left:
            return AlignmentAxis(alignment: HorizontalAlignment.left, position: inset.left)
        case .right:
            guard let collectionViewWidth = collectionView?.frame.size.width else {
                return nil
            }
            return AlignmentAxis(alignment: HorizontalAlignment.right, position: collectionViewWidth - inset.right)
        default:
            return nil
        }
    }
    ...
    fileprivate func isFrame(for firstItemAttributes: UICollectionViewLayoutAttributes, inSameLineAsFrameFor secondItemAttributes: UICollectionViewLayoutAttributes) -> Bool {
        let lineWidth = contentWidthForSection(at: firstItemAttributes.indexPath.section)
        if lineWidth == 0.0 {
            return false
        }
        let firstItemFrame = firstItemAttributes.frame
        let lineFrame = CGRect(x: sectionInsetForSection(at: firstItemAttributes.indexPath.section).left,
    ...
    fileprivate func layoutAttributes(forItemsInLineWith layoutAttributes: UICollectionViewLayoutAttributes) -> [UICollectionViewLayoutAttributes] {
        let lineWidth = contentWidthForSection(at: layoutAttributes.indexPath.section)
        if lineWidth == 0.0 {
            return [layoutAttributes]
        }
        var lineFrame = layoutAttributes.frame
        lineFrame.origin.x = sectionInsetForSection(at: layoutAttributes.indexPath.section).left
...
fileprivate extension UICollectionViewLayoutAttributes {
    ...
    func align(toAlignmentAxis alignmentAxis: CollectionViewAlignedFlowLayout.AlignmentAxis<CollectionViewAlignedFlowLayout.HorizontalAlignment>) {
    ...
    func align(toAlignmentAxis alignmentAxis: CollectionViewAlignedFlowLayout.AlignmentAxis<CollectionViewAlignedFlowLayout.VerticalAlignment>) {
        switch alignmentAxis.alignment {
        case .top:
    ...
    private func alignToPrecedingItem(collectionViewLayout: CollectionViewAlignedFlowLayout) {
        let itemSpacing = collectionViewLayout.minimumInteritemSpacingForSection(at: precedingIndexPath.section)
    ...
    private func alignToFollowingItem(collectionViewLayout: CollectionViewAlignedFlowLayout) {
        let itemSpacing = collectionViewLayout.minimumInteritemSpacingForSection(at: followingIndexPath.section)
    ...
    func alignHorizontally(collectionViewLayout: CollectionViewAlignedFlowLayout, forSectionAt section: Int) {
        
        guard let alignmentAxis = collectionViewLayout.alignmentAxisForSection(at: section) else {
            return
        }
